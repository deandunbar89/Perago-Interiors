...
export default function GanttView({ tasks }: { tasks: UnifiedTaskRow[] }) {
  ...
  const dates = dated.map((t) => t.dueDate as Date);
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="overflow-x-auto">
        <div style={{ width: timelineWidth + 220 }}>
          <div className="flex border-b border-slate-100 text-xs font-semibold uppercase tracking-wide text-slate-500">
            <div className="w-[220px] shrink-0 px-4 py-2.5">Task</div>
          ...
          <div className="divide-y divide-slate-100">
            {dated.map((task) => {
              const due = startOfDay(task.dueDate as Date);
              const overdue = task.status === "OPEN" && isPast(due) && !isToday(due);
              ...
              return (
                <div key={task.id} className="flex items-center">
                  ...
                  <div className="relative flex-1" style={{ width: timelineWidth, height: 36 }}>
                    ...
                    <div
                      title={`${task.title}: ${format(due, "MMM d, yyyy")}`}
                      className={`absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rotate-45 ${color} opacity-90`}
                      style={{ left }}
                    />
                  </div>
                </div>
      ...
      <p className="border-t border-slate-100 px-4 py-2 text-xs text-slate-400">
        Each marker sits on the task&apos;s due date.
      </p>
    </div>
  );
}
