...
import { ALL_COLUMNS, COLUMN_LABELS, DEFAULT_COLUMNS, sortPmProjects, type ColumnId, type SortState } from "./columns";
import type { PmProjectRow } from "./types";

type View = ExplorerView;

const VIEW_KEY = "tendercrm.pm.projects.view";
const COLUMNS_KEY = "tendercrm.pm.projects.columns";
const SORT_KEY = "tendercrm.pm.projects.sort";

export default function PmProjectsExplorer({
  projects,
  clients,
}: {
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter((p) => {
      ...
      if (search) {
        const haystack = [p.title, p.location, p.client?.name].filter(Boolean).join(" ").toLowerCase();
  ...
  const sorted = useMemo(() => sortPmProjects(filtered, sort), [filtered, sort]);
