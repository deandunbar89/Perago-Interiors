...
function getBarRange(project: ProjectRow): [Date, Date] {
  const start = project.siteVisitDate || project.createdAt;
  let end = project.awardDate || project.submissionDeadline || start;
...
export default function GanttView({ projects }: { projects: ProjectRow[] }) {
  ...
  const minDate = startOfMonth(
    new Date(Math.min(...ranges.map(([s]) => s.getTime())))
  );
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div style={{ width: timelineWidth + 220 }}>
          ...
          <div className="flex border-b border-slate-100 text-xs font-semibold uppercase tracking-wide text-slate-500">
            ...
            <div className="relative flex-1" style={{ width: timelineWidth }}>
              {months.map((m) => {
                const offset = differenceInCalendarDays(m, minDate) * DAY_WIDTH;
                return (
                  <div
                    key={m.toISOString()}
      ...
      <p className="border-t border-slate-100 px-4 py-2 text-xs text-slate-400">
        Bars run from site visit (or creation date) to award date or submission deadline.
      </p>
    </div>
  );
}
